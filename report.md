![Ar-Rasheed University Logo](Ar-rasheed_uni_logo.png)

# 1. Visual Prompt Injection Detection in Large Vision-Language Models

**Author:** Odai Gubran  
**Course Context:** Project Submission for Eng. Sondos Saif  
...
flowchart TD
    ...
    classDef cv fill:#bbf,stroke:#333,stroke-width:2px;
    classDef nlp fill:#bfb,stroke:#333,stroke-width:2px;
    classDef ml fill:#fbb,stroke:#333,stroke-width:2px;
    classDef verdict fill:#eee,stroke:#333,stroke-width:4px;

    %% Nodes
    A[Raw Image Input \n Synthetic / CyberSecEval 3]:::input
    
    subgraph CV [Computer Vision Engine]
        B(Grayscale Conversion)
        C(Contrast Stretching)
        D(Otsu Adaptive Binarization \n Defeats Camouflage)
        E(4-Tier Fallback OCR Wrapper)
    end
    
    subgraph NLP [Natural Language Processing]
        F(Lexical Normalization)
        G(Security Token Preservation \n e.g., '### Instruction')
        H(TF-IDF N-gram Vectorization)
    end
    
    subgraph ML [Machine Learning Classification]
        I(Linear SVM / Random Forest)
        J(Probability Calibration)
    end
    
    K{Security Verdict}:::verdict
    L[Safe Content]:::input
    M[Malicious Payload Detected!]:::input

    %% Connections
    A --> CV:::cv
    B --> C --> D --> E
    CV -->|Extracted Raw Text| NLP:::nlp
    F --> G --> H
    NLP -->|Sparse TF-IDF Matrix| ML:::ml
    I --> J
    ML --> K
    K -->|Benign Class| L
    K -->|Injection Class| M
...
1. **The 20% Holdout Synthetic Test Set.**
...
## 13. References
1. Meta (2024). *CyberSecEval 3: Advancing the Evaluation of Cybersecurity Risks in Large Language Models.* Hugging Face.
2. scikit-learn developers (2024). *scikit-learn: Machine Learning in Python.*
3. Smith, R., et al. (2007). *An Overview of the Tesseract OCR Engine.*
